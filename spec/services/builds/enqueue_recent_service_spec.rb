...
describe Builds::EnqueueRecentService do
  let(:user) { create :user }
  let(:repo) { create :repo }
  let(:branch) { create :branch, repo: repo }
  let!(:launch_job) { class_spy(Builds::LaunchJob, meta_id: 'meta-id').as_stubbed_const }
  ...
  context 'when has no recent revision' do
    let(:service) { Builds::EnqueueRecentService.new(user, repo) }
  ...
  context 'when has build with same recent revision' do
    let(:service) { Builds::EnqueueRecentService.new(user, repo, branch_name: branch.name) }
